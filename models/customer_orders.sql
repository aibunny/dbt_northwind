WITH customer_orders AS (
  ...
  FROM
    {{ ref('postgres.order_details') }} o
    JOIN {{ ref('postgres.customers') }} c ON o.customer_id = c.customer_id
    LEFT JOIN {{ ref('postgres.order_details') }} od ON o.order_id = od.order_id
  GROUP BY
